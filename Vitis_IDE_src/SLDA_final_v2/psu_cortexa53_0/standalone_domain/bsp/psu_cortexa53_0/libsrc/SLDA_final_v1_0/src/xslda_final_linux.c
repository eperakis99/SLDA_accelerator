...
typedef struct {
    ...
    XSlda_final_uio_map maps[ MAX_UIO_MAPS ];
} XSlda_final_uio_info;

/***************** Variable Definitions **************************************/
static XSlda_final_uio_info uio_info;
...
static int uio_info_read_name(XSlda_final_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSlda_final_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSlda_final_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSlda_final_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSlda_final_Initialize(XSlda_final *InstancePtr, const char* InstanceName) {
	XSlda_final_uio_info *InfoPtr = &uio_info;
